// Mock implementation for demo (replace with actual import when using a bundler)
// import { getEmailProvider } from '@mikkelscheike/email-provider-links';

// Mock function to demonstrate the library (replace this with the real import)
async function getEmailProvider(email, timeout = 5000) {
    ...
    const providers = {
        ...
        'outlook.com': {
            provider: { companyProvider: 'Microsoft Outlook' },
        ...
        'yahoo.com': {
            provider: { companyProvider: 'Yahoo Mail' },
            loginUrl: 'https://mail.yahoo.com/',
            email: email,
            detectionMethod: 'domain_match'
        },
        'protonmail.com': {
            provider: { companyProvider: 'ProtonMail' },
            loginUrl: 'https://mail.proton.me/',
            email: email,
            detectionMethod: 'domain_match'
        },
        'icloud.com': {
            provider: { companyProvider: 'iCloud Mail' },
    ...
    // Simulate DNS detection for business domains
    if (!['gmail.com', 'outlook.com', 'yahoo.com', 'hotmail.com', 'live.com', 'icloud.com', 'protonmail.com'].includes(domain)) {
        // Simulate some business domains
        const businessProviders = {
            'example.com': 'Google Workspace',
            'mycompany.com': 'Microsoft 365',
            'business.org': 'Google Workspace',
            'corp.net': 'Microsoft 365'
        };
        
        if (businessProviders[domain]) {
            return {
                provider: { companyProvider: businessProviders[domain] },
                loginUrl: businessProviders[domain].includes('Google') ? 
                    'https://mail.google.com/mail/' : 'https://outlook.office.com/',
                email: email,
...
async function handleEmailChange() {
    ...
    const domain = email.split('@')[1].toLowerCase();
    const commonDomains = ['gmail.com', 'outlook.com', 'yahoo.com', 'hotmail.com', 'icloud.com', 'protonmail.com'];
    ...
    if (currentDetectedProvider && currentDetectedProvider.provider) {
        ...
        const resultData = JSON.stringify({
            provider: {
                ...
                // Add other relevant provider fields if available
            },
            email: currentDetectedProvider.email,
            loginUrl: currentDetectedProvider.loginUrl,
            detectionMethod: currentDetectedProvider.detectionMethod,
            // Add other fields as they exist in your library response
        }, null, 2);
...
console.log('🚀 Email Provider Links Demo loaded!');
